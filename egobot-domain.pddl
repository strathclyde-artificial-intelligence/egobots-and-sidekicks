...
;the not-handled precondition weeds out ineffective strategies in which a tool is picked up and dropped multiple time by the same agent in Egobot plans, and should not be included in full problem solving
(:predicates 
    (hands-free ?r - robot)
    (camera-free ?r - robot)
...
(:durative-action drop-welder
    :parameters (?r - robot ?t - welder ?l - location)
    :duration (= ?duration 1)
    :condition (and 
        (at start (and 
            (holding ?r ?t)
...
(:durative-action drop-patch
    :parameters (?r - robot ?t - patch ?l - location)
    :duration (= ?duration 1)
    :condition (and 
        (at start (and 
            (holding ?r ?t)
