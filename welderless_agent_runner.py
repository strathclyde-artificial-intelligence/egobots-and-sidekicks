...
def hardness(results_directory):
    dirsep = results_directory.split('_')
    egobotstr = dirsep[2]
    locationstr = dirsep[3]
    egobotint = int(egobotstr[1:])
    locationint = int(locationstr[1:])
    hardness_value = egobotint + locationint
...
for dir in dirs:
    ...
    if testcomplete == 1:
        welderless_agent_file_list = [f for f in os.listdir(dir) if "Welderless" in f]
        if welderless_agent_file_list == []:
            ...
            fullplanfile = filecode+'Welderless-Agent-Plan.txt'
            fulldomain = 'maintenance-domain-2-welderless.pddl'
